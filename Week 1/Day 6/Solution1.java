...
public class Solution {
   ...
   public static int maxDuffelBagValue(final CakeType[] cakes, final int capacity) {
    final int[] maxCapacity= new int[capacity + 1];
    ...
    for (int i=0;i<=capacity;i++) {
      ...
      for (int j=0;j<cakes.length;j++) {
        ...
        if (cake.weight<=i) {
          ...
          int maxValue = cake.value + maxCapacity[i - cake.weight];
          currentMax= Math.max(maxValue, currentMax);
        
        }
      }
      maxCapacity[i] = currentMax;
